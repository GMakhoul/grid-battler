...
- **Online PvP (2 devices):** `python relay.py` (aiohttp WS relay, port 8765, rooms by `?room=CODE`). Title → "PvP online (com amigo)" → **room browser** (see below). First peer = host (runs the sim, host-authoritative), second = guest.
- **Matchmaking = room browser (CS-1.6 style, 2026-06-23):** the relay tracks open rooms (exactly 1 peer waiting) and exposes `GET /rooms` → `{rooms:[{code,name,color,wait}]}`. `rooms` dict is now `{code:{peers:[...],name,color,ts}}`. Host's display name (character `nm`) + color ride in on the WS connect query (`?room=&name=&color=`). Client: `#screen-net` shows a **live list** (polls `/rooms` every 2.5s while `cur==="net"`; `startRoomPolling`/`stopRoomPolling` hooked in `show()` + `netBattleUI`), each row has a **"entrar"** button → `connectNet(code)`. "criar sala" → `connectNet(newRoomCode(), {name,color})`. Code entry kept as a `<details>` "sala privada" fallback. **Untrusted `name`/`color` from other players are sanitized on render** (`textContent`, not innerHTML; `safeColor()` hex-validates). No code typing needed for normal play.
...
| `relay.py` | aiohttp WS relay for online PvP + `GET /rooms` (open-room list for the browser) + `GET /` (game, `Cache-Control: no-cache`). Rooms = `{code:{peers,name,color,ts}}`. Reads `PORT` from env (cloud) / 8765 local. |
...
- **Card cycle = Clash Royale deterministic queue (2026-06-23, replaced random draw):** `HAND_SIZE=4` + a **"próxima"** preview (`#nextCard`, non-playable). `dealHand(s)` shuffles the deck ONCE → first 4 to `s.hand`, rest to `s.cycle` (`cycle[0]` = the shown next). Firing/fusing → `cycleCard(s,i,spent)`: the next (`cycle.shift()`) fills the vacated slot, the spent card(s) go to `cycle.push(...)` (back). No randomness, no draw timer; pacing is the energy gate. **Fusion keeps the 8-deck whole:** a fused card stores `parts` = its underlying BASE cards (accumulates through triples, e.g. zetacannon → `[cannon,cannon,grenade]`); firing it returns all bases to the cycle. This is why the exactly-8 deck rule matters (4 hand + 4 cycle). `drawSpeed` is now vestigial (still plumbed in cfgs, read nowhere).
- **Netcode = host-authoritative**, 30Hz snapshots. `makeSnap`/`applySnap`, `sideSnap` (incl. `hand` + `next` = the cycle's front card; the guest doesn't simulate the cycle so it needs `next` to draw the preview), `effSnap` (draw-only, strips circular `atk`/`foe` refs).
- **Resolution independence (2026-06-23):** each device computes its own `cell` (grid size from screen), so PIXEL coords in a snapshot (proj `x/y`, floaters `x/y/dx/dy`, fx rings `x/y/r0/r1`, `shake.mag`) are in the HOST's pixels. `makeSnap` sends `cell` and `applySnap` rescales every pixel field by `cell/s.cell`. **Rule: any new snapshot field carrying a pixel value must be scaled in `applySnap`** (tile/`col`/`row`-based effects are exempt — they recompute from indices with the local cell). This fixed the guest seeing projectiles/numbers/rings shifted (verified: host px150@cell100 drew at 147 on a cell-98 guest). Guest renders snapshots only (no sim) and sends `{t:'in',...}` inputs. **Any new per-side or per-effect field that the guest must see has to be added to the snapshot functions** (e.g. `effSnap` carries `rows` for yoyo). Input always routed through `cmdMove/cmdFire/cmdFuse`.
...
| Tático ♛ | 800 | .16 | 1.0 | .70 | maxEnergy 12 (energy-pool identity; old `drawSpeed` retired with the deterministic cycle) |
...
- **iOS zoom — standalone/app fix (2026-06-23):** the JS pinch/double-tap blockers + `user-scalable=no` aren't enough on **Safari iOS** (Apple ignores `user-scalable` for a11y; Chrome iOS still honors it → why the friend's Chrome workaround works). Added: `apple-mobile-web-app-capable`/`mobile-web-app-capable`/status-bar-style/title metas + `theme-color`, so **"Adicionar à Tela de Início" → launches standalone, no Safari zoom gestures** (the real fix). Plus `touch-action: manipulation` on `*` (kills in-tab double-tap-zoom; arena stays `none`, scroll `pan-y`, input `auto`). Verified via DOM (metas present, btn=manipulation, arena=none). **In a normal Safari tab, 100% zoom prevention is impossible** — home-screen install is the clean path. Optional later: a real manifest + apple-touch-icon (PNG) for a polished install icon.
...
- **Hosting (2026-06-23):** DONE & LIVE — pushed to GitHub `GMakhoul/grid-battler` and deployed on **Render free** (single deploy of `relay.py` serves game + relay; client's `wss://<host>/ws` works unchanged). Deploy bundle: `requirements.txt`/`render.yaml`/`DEPLOY.md`; `relay.py` reads `PORT`. cloudflared remains the quick local-tunnel fallback. Caveat: free instance sleeps ~15 min idle (~50s cold start).
- **Card cycle → Clash Royale (2026-06-23):** replaced the random per-slot draw-timer with a deterministic 8-card cycle (hand 4 + `#nextCard` preview; `dealHand`/`cycleCard`; fused cards carry `parts` to return bases to the cycle; `next` added to `sideSnap`). Verified live (375×812 viewport so `elementFromPoint` works — headless is 0×0 otherwise): single-slot firing gave a clean periodic sequence with the "próxima" preview predicting the drawn card **every** time (period-4 = the 4-card cycle); fusion produced megacannon and firing it returned both base cannons (no slot ever emptied over 13+ fires); 0 console errors. **Needs real-playtest feel:** whether hand-of-4 (down from 5) feels too tight for assembling fusion pairs.
- **Matchmaking room browser (2026-06-23):** replaced create-room/type-code with a live `/rooms` list (CS-1.6 style) — verified end-to-end: server `/rooms` lists 1-peer rooms & hides full ones (simulated WS host/guest); client renders rows with name/color/wait + working "entrar"; XSS-safe (escaped name, hex-validated color); empty/offline states; own-room filtered; polling stops off-screen; and a **real client→relay create** showed up in the live `/rooms` with the right name+color. (`wait` shows a huge baseline in-sandbox due to the unstable VM clock — `int(now-ts)` is correct, +3s per 3s; fine on Render.) **Caching note:** old browsers may still show the pre-`no-cache` build until one hard refresh.
- **Cache-Control (2026-06-23):** `index.html` now served with `no-cache` so each deploy shows fresh (revalidates via ETag → 304 when unchanged). Fixes "same URL shows old version".
- **Guest visual misalignment FIXED (2026-06-23):** the joiner saw projectiles/damage-numbers/impact-rings shifted (often left) while hits still registered — because those are PIXEL-based and were drawn at the host's `cell` scale on a different-sized screen. Fix = `makeSnap` sends `cell`, `applySnap` rescales pixel fields by `cell/s.cell`. Verified via a Python host streaming a crafted snapshot (proj x150@cell100 → guest drew at 147 @cell98), 0 errors. **User reported it on real 2-phone play; the real measured ping was 440ms on Render vs 30-50ms on the local relay (Cloudflare tunnel) — see latency below.**
- **Ping meter + latency (2026-06-23):** added a live RTT meter — both sides send `{t:"ping",ts:performance.now()}` every 1s through the relay; the peer echoes `{t:"pong",ts}`; sender computes `now-ts` (no clock-sync needed) and shows `📶 Xms` (green<80/yellow<160/red) next to the opponent name (`#pingTag`, net-only). `startPing` in `netBattleUI`, `stopPing` at top of `show()`. Verified end-to-end via a Python WS guest looping back pong → host showed `📶 1ms` (loopback) green, 0 errors. **Latency — measured (2026-06-23):** user reports **Render = 440ms (unplayable)**, **local relay via Cloudflare tunnel = 30-50ms (great)**. Root cause: host-authoritative + **relay** means every gameplay msg crosses the cloud TWICE (peer→relay→peer), and Render has **no South America region** (user is in Brazil) → the double-hop × the Brazil↔US distance ≈ 440ms. Levers, by impact: (1) **host the relay in São Paulo** — Render can't (no SA region), but **Fly.io has a `gru` (São Paulo) region** → would cut Brazilian RTT to ~80-160ms; the relay is portable (it's just `relay.py`/aiohttp). (2) **same-Wi-Fi → local relay** (~1-5ms, already works). (3) **guest-side client prediction** (predict own move/cast locally, reconcile w/ snapshots — NOT done; helps feel but doesn't lower the number). (4) WebRTC P2P cuts the double-hop but needs STUN/TURN — overkill. **Recommended next: deploy the relay to Fly.io GRU** (biggest, lowest-risk win for BR players); offered to prep the Fly config. Prediction is the follow-up if still needed.
