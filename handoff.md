...
- **Netcode = host-authoritative**, 30Hz snapshots. `makeSnap`/`applySnap`, `sideSnap` (incl. `hand` + `next` = the cycle's front card; the guest doesn't simulate the cycle so it needs `next` to draw the preview), `effSnap` (draw-only, strips circular `atk`/`foe` refs). Guest renders snapshots only (no sim) and sends `{t:'in',...}` inputs. **Any new per-side or per-effect field that the guest must see has to be added to the snapshot functions** (e.g. `effSnap` carries `rows` for yoyo). Input always routed through `cmdMove/cmdFire/cmdFuse`.
...
- **Ping meter + latency (2026-06-23):** added a live RTT meter — both sides send `{t:"ping",ts:performance.now()}` every 1s through the relay; the peer echoes `{t:"pong",ts}`; sender computes `now-ts` (no clock-sync needed) and shows `📶 Xms` (green<80/yellow<160/red) next to the opponent name (`#pingTag`, net-only). `startPing` in `netBattleUI`, `stopPing` at top of `show()`. Verified end-to-end via a Python WS guest looping back pong → host showed `📶 1ms` (loopback) green, 0 errors. **Latency levers (explained to user, not yet done):** the sim is host-authoritative + cloud-relayed, so the GUEST eats a full guest→Render→host→Render→guest round-trip (host is ~0). Biggest free win = **Render region closest to Brazil (US-East: Ohio/Virginia, not Oregon)**; same-Wi-Fi = run the **local relay** (~1-5ms); the real feel fix = **guest-side client prediction** (predict own move/cast locally, reconcile with snapshots — still NOT done, flagged as the next polish); WebRTC P2P would cut the double-hop but is overkill for now.
